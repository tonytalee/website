...
### 輸入數據
數據內容先用試算(例如 excel 或 google sheet) 輸入數據再儲存成 csv 格式。再利用側邊欄的 'Select a CSV file' 選擇要分析的 csv 檔。

**數據格式：**
csv 檔的第一列為欄位名稱，數據從第二列開始。

* 單一樣本群組 (one-sample)：將數據儲存在第一欄。
* 兩組樣本群組 (two-sample 及 paired-sample)：包含三個欄位。第一個欄位為每個樣本的 id，每一列為一個樣本, id 不可重複，若無特定 id, 編上流水號也可以。第二欄為樣本的值。第三欄為樣本的群組別，因為是 two-sample，所以應只有兩個群組別。欄位順序可以互換，在介面中，會選擇 id, 樣本值跟群組別相對應的欄位。
...
若是選擇帶樣批次的原始數據，程式會先用 p-chart 檢查 批次間的穩定性，若有不穩定的批次，務必要確認。
